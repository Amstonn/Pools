# 内存池
## 内存管理
1. 内存管理实质上管理的是虚拟内存，可以操作的是堆和共享内存。
2. 动态创建的对象在堆中，例如malloc(5)就会申请在堆上，但是容易产生碎片，所以将内存空间拆分为固定大小的块。
## 内存池
内存池是一种内存管理的技术，主要用于优化动态内存分配的效率和性能。主要包括：减少内存碎片  提高分配效率   避免频繁系统调用  特定内存对齐  内存回收等。
